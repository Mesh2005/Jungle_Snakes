...
/**
 * Resolve a CSS color variable to hex for WebGL.
 * Handles both hex (#a3ff3c) and computed rgb(r, g, b) from the document.
 */
...
/** Read --theme-accent from the document (theme-responsive). */
function getThemeAccentHex(): string {
  if (typeof document === 'undefined') return '#a3ff3c';
  const value = getComputedStyle(document.documentElement).getPropertyValue('--theme-accent').trim();
  return value ? cssColorToHex(value) : '#a3ff3c';
}
...
export interface LightRaysProps {
  /** Starting point of the light rays. Default: top-center */
  raysOrigin?: RaysOrigin;
  /** Primary color of the rays (hex). If not set, uses --theme-accent (theme-responsive). */
  raysColor?: string;
  ...
  saturation?: number;
  /** Whether rays tilt towards the mouse */
  followMouse?: boolean;
...
export function LightRays({
  ...
  const raysColor = raysColorProp ?? getThemeAccentHex();
  ...
  useEffect(() => {
    ...
    const initializeWebGL = async () => {
      ...
      const loop = (t: number) => {
        ...
        if (raysColorProp === undefined) {
          const themeColor = getThemeAccentHex();
          uniforms.raysColor.value = hexToRgb(themeColor);
        }
